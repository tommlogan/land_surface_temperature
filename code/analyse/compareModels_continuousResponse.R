...
#   pcaData$response = theData$response
  ...
  modelNames = c("meanOnly", "glm1",  "gam1","mars1","tree1", "bag1", "rf1", "boost1", "nnet1", "glm2", "mars2","tree2", "bag2", "rf2", "boost2","nnet2")#,"glm3")#,"bart1")
...
#     rf2 = randomForest(response ~ ., data = trainData,  mtry = 22, importance = T)
    ...
    # init the rows
    mse.row = matrix(data=NA, nrow = 1, ncol = numModels)
    colnames(mse.row) = modelNames
    mae.row = matrix(data=NA, nrow = 1, ncol = numModels)
    colnames(mae.row) = modelNames
    mpe.row = matrix(data=NA, nrow = 1, ncol = numModels)
    colnames(mpe.row) = modelNames
    # add to results rows
    for (n in modelNames){
      # get the prediction data for the model
      predicted = get(paste('pred_',n,sep = ""))
      # calculate the MSE of the predictions
      mse.row[1,n] <-  mean((testData$response - predicted)^2)
      mae.row[1,n] <- mean(abs(testData$response - predicted))
      mpe.row[1,n] <- mean(abs(testData$response - predicted)/testData$response)
      # R2Table[i,n] = sum((predicted-mean(testData$response))^2)/sum((testData$response-mean(testData$response))^2) 
    }
  results.rows = list(mse = mse.row, mae = mae.row, mpe = mpe.row)
  return(results.rows)
  }
  
  # parallelise the holdouts
  cl <- makeCluster(no_cores,outfile="")
  clusterExport(cl, c("holdout","theData","numObs","z","modelNames","fmlaVif5","numModels","fmlaVif5GAM","glmVIF5"), envir=environment())
  # clusterEvalQ(cl)
  error.tables = pblapply(seq(1,k),function(i) holdout(i),cl = cl)
  stopCluster(cl)
  
  # now separate the error.rows into error tables
  for (i in seq(1,k)){
    mseTable[i,] = error.tables[[i]]$mse
    maeTable[i,] = error.tables[[i]]$mae
    mpeTable[i,] = error.tables[[i]]$mpe
  }
  ...
  # plot the bar charts 
  #par(mfrow=c(1,2))
  barMSE = barplot(mseTable[k+1,],xlab="Models",ylab="mean square error",las=2)
...
mseTable2 = mseTable[,c(1,3,6,10,12,18)]
maeTable2 = maeTable[,c(1,3,6,10,12,18)]
  # box plots
  boxplot(mseTable2,xlab="Models",ylab="mean square error",las=2)
  boxplot(maeTable2,xlab="Models",ylab="mean absolute error",las=2)
  boxplot(mpeTable*100,xlab="Models",ylab="mean percentage error",las=2)
